...
<div class="stats-grid">
    <div class="stat-card">
        ...
        <div style="margin-top: 1rem; height: 4px; background: var(--color-surface-muted); border-radius: 2px;">
            ...
            ?>
            <div style="height: 100%; width: <?= $progress ?>%; background: var(--color-accent); border-radius: 2px;">
            </div>
        </div>
    ...
    <div class="stat-card">
        ...
        <span class="stat-sub" style="color: var(--color-warning);"><?= (int) ($counts['half_day'] ?? 0) ?> half-days
            recorded</span>
    </div>
    ...
    <div class="stat-card">
        ...
        <div class="stat-card" style="border: 1px solid var(--color-accent); position: relative; overflow: hidden;">
            <div
                style="position: absolute; top: -10px; right: -10px; width: 60px; height: 60px; background: var(--color-accent-soft); border-radius: 50%; z-index: 0;">
            </div>
            <span class="stat-label" style="position: relative; z-index: 1;">Monthly Hourglass</span>
            <div style="display: flex; gap: 0.75rem; align-items: baseline; position: relative; z-index: 1;">
                <span class="stat-value"><?= esc($totalHoursMonth) ?>h</span>
                <span class="text-muted" style="font-size: 0.875rem;">logged</span>
            </div>

            <div
                style="margin-top: 1rem; height: 6px; background: var(--color-surface-muted); border-radius: 3px; overflow: hidden; position: relative; z-index: 1;">
                <?php
                $monthProgress = min(($totalHoursMonth / $requiredHoursMonth) * 100, 100);
                ?>
                <div
                    style="height: 100%; width: <?= $monthProgress ?>%; background: linear-gradient(90deg, var(--color-accent) 0%, #6366f1 100%); border-radius: 3px;">
                </div>
            </div>

            <div
                style="margin-top: 0.5rem; display: flex; justify-content: space-between; align-items: center; position: relative; z-index: 1;">
                <span
                    style="font-size: 0.7rem; font-weight: 700; color: var(--color-accent); text-transform: uppercase;"><?= round($monthProgress) ?>%
                    Completed</span>
                <span style="font-size: 0.7rem; font-weight: 600; color: var(--color-text-dim);">Goal:
                    <?= esc($requiredHoursMonth) ?>h</span>
            </div>
        </div>
    </div>
