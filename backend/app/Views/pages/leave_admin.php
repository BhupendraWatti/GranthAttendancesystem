    ...
    <div style="display: flex; justify-content: space-between; align-items: flex-end;">
        ...
        <div style="display: flex; gap: 1rem; align-items: center;">
            <div style="text-align: right;">
                ...
                <div style="font-size: 1.5rem; font-weight: 800; color: var(--color-warning);">
                    <?= count($pending ?? []) ?>
                </div>
            </div>
            <div
                style="width: 48px; height: 48px; border-radius: var(--radius-md); background: linear-gradient(135deg, var(--color-warning) 0%, #FBBF24 100%); display: flex; align-items: center; justify-content: center; color: white; font-size: 1.25rem;">
...
<div class="card animate-in"
    ...
    <div class="card-header">
        <div style="display: flex; align-items: center; gap: 0.75rem;">
            <div
                style="width: 40px; height: 40px; border-radius: var(--radius-md); background: linear-gradient(135deg, var(--color-warning) 0%, #FBBF24 100%); display: flex; align-items: center; justify-content: center; color: white;">
                <i class="fa-solid fa-hourglass-half"></i>
            </div>
            <div>
                <h3 style="margin: 0;">Pending Approval</h3>
                <div style="font-size: 0.75rem; color: var(--color-text-dim);"><?= count($pending ?? []) ?> requests
                    awaiting review</div>
            </div>
        </div>
    </div>
            ...
            <table style="border: none;">
                <thead>
                    <tr style="background: linear-gradient(135deg, var(--color-warning) 0%, #FBBF24 100%);">
                        <th style=" border: none; padding: 1rem 1.5rem;">Employee</th>
                        <th style=" border: none; padding: 1rem 1.5rem;">Duration</th>
                        <th style=" border: none; padding: 1rem 1.5rem;">Status</th>
                        <th style=" border: none; padding: 1rem 1.5rem;">Admin Comment</th>
                    </tr>
                </thead>
                <tbody>
                            ...
                            <tr style="border-bottom: 1px solid var(--color-border); transition: all 0.2s;">
                                <td style="padding: 1.25rem 1.5rem;">
                                    <div style="display: flex; align-items: center; gap: 1rem;">
                                        ...
                                        <div>
                                            <div style="font-weight: 700; color: var(--color-primary); font-size: 0.9375rem;">
                                                <?= esc($p['employee_name']) ?>
                                            </div>
                                ...
                                <td style="padding: 1.25rem 1.5rem;">
                                    <div style="font-weight: 600; color: var(--color-primary); font-size: 0.9375rem;">
                                        <?= date('d M', strtotime($p['from_date'])) ?> -
                                        <?= date('d M Y', strtotime($p['to_date'])) ?>
                                    </div>
                                ...
                                <td style="padding: 1.25rem 1.5rem;">
                                    <div style="max-width: 250px; font-size: 0.875rem; color: var(--color-text-main); line-height: 1.5;"
                                        title="<?= esc($p['reason']) ?>">
                                        <?= esc($p['reason']) ?>
                                    </div>
                                </td>
                                <td style="padding: 1.25rem 1.5rem; text-align: right;">
                                    <div style="display: flex; gap: 0.5rem; justify-content: flex-end;">
                                        <button class="btn btn-primary"
                                            style="padding: 0.5rem 1rem; font-size: 0.8rem; gap: 0.5rem; background: linear-gradient(135deg, var(--color-success) 0%, #34D399 100%);"
                                            onclick="handleAction('approve', <?= $p['id'] ?>)">
                                            <i class="fa-solid fa-check"></i>
                                            Approve
                                        </button>
                                        <button class="btn btn-outline"
                                            style="padding: 0.5rem 1rem; font-size: 0.8rem; gap: 0.5rem; color: var(--color-danger); border-color: var(--color-border);"
                                ...
                                <div class="empty-state"
                                    ...
                                    <div
                                        ...
                                        <i class="fa-solid fa-check-circle"
                                            style="font-size: 2rem; color: var(--color-success);"></i>
                                    </div>
                                    <div>
                                        <div
                                            style="font-weight: 700; color: var(--color-text-dim); font-size: 1.125rem; margin-bottom: 0.5rem;">
                                            All caught up!</div>
                                        <p style="font-size: 0.875rem; color: var(--color-text-dim);">No pending leave
                                            requests to review at this time.</p>
                                    </div>
                                </div>
                            </td>
                        </tr>
...
<div class="card animate-in" style="animation-delay: 0.2s;">
    <div class="card-header">
        <div style="display: flex; align-items: center; gap: 0.75rem;">
            <div
                style="width: 40px; height: 40px; border-radius: var(--radius-md); background: linear-gradient(135deg, var(--color-primary) 0%, var(--color-primary-light) 100%); display: flex; align-items: center; justify-content: center; color: white;">
                <i class="fa-solid fa-clipboard-list"></i>
            </div>
            <div>
                <h3 style="margin: 0;">Request History</h3>
                <div style="font-size: 0.75rem; color: var(--color-text-dim);"><?= count($history ?? []) ?> processed
                    requests</div>
            </div>
        </div>
    </div>
                    ...
                    <?php if (!empty($history)): ?>
                        <?php foreach ($history as $h): ?>
                            <tr style="border-bottom: 1px solid var(--color-border); transition: all 0.2s;">
                                ...
                                <td style="padding: 1.25rem 1.5rem;">
                                    <div style="font-weight: 600; color: var(--color-primary); font-size: 0.9375rem;">
                                        <?= date('d M', strtotime($h['from_date'])) ?> -
                                        <?= date('d M Y', strtotime($h['to_date'])) ?>
                                    </div>
                                    <div style="font-size: 0.75rem; color: var(--color-text-dim); margin-top: 0.25rem;">
                                ...
                                <div class="empty-state"
                                    ...
                                    <div>
                                        <div
                                            style="font-weight: 700; color: var(--color-text-dim); font-size: 1.125rem; margin-bottom: 0.5rem;">
                                            No history yet</div>
                                        <p style="font-size: 0.875rem; color: var(--color-text-dim);">Processed leave
